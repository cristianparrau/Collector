...
from src.database import init_db, get_session_factory, RecordModel, ExecutionRunModel, get_db
from src.services.llm_service import ask_database
...
def ask_question(payload: QuestionRequest, db: Session = Depends(get_db)):
    answer_text, sources = ask_database(db, payload.question)
...
def trigger_collection(
    max_records: int = Query(None, description="Sobrescribe el límite de registros")
):
    try:
